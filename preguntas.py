...
def pregunta_01():
    ...
    sep='\t',
    header=None,
    names=['mensaje', 'etiqueta'],
)

    # Separe los grupos de mensajes etiquetados y no etiquetados.
    df_tagged = df[df["etiqueta"].notnull()]
    df_untagged = df[df["etiqueta"].isnull()]
...
def pregunta_03():
    ...
    stemmer = PorterStemmer() # Reduccion a la raiz
...
def pregunta_04():
    ...
    countVectorizer = CountVectorizer(
        analyzer=analyzer,
        lowercase=True,
        stop_words='english', # Palabras que no aportan significado
        token_pattern=r'\b[a-zA-Z][a-zA-Z]+\b', # Al menos dos letras para ser procesada
        binary=True, # True para ver si está o no, no importa cuantas
        max_df=1.0, # Frecuencia maxima de 100
        min_df=5, # Frecuencia minima que debe aparecer
    )
    ...
    # BernoulliNB.
    param_grid = {
        "Bernoulli__alpha": np.linspace(0.1, 1.0, num = 10),
    }
